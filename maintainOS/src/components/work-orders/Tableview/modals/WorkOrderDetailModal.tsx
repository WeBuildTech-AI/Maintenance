...
export default function WorkOrderDetailModal({
  ...
  useEffect(() => {
    ...
    const fetchMissingProcedures = async () => {
      ...
      // ✅ Optimization: Check 'safeWorkOrder' or just trust existing caching checks
      ...
      if (needsFetch) {
        try {
          ...
          if (fetchedProcedures.length > 0 && fetchedProcedures[0].id === workOrder.procedureIds[0]) {
            console.log("⚡ [Optimization] Reusing fetched procedures");
            return;
          }

          // Guarded Timer
          const TIMER_LABEL = "PROCEDURES_FETCH_MODAL";
        ...
        // setFetchedProcedures([]); // Don't reset if we want to keep cache? 
        // Actually if we switch WOs, we should reset or rely on the dependency array.
        if (workOrder.id !== fetchedProcedures[0]?.workOrderId) { // hypothetical check, relying on deps is safer
                    ...
                    {(workOrder.assetStatus || (workOrder.assets && workOrder.assets.length > 0)) && (
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          <div className={`h-2.5 w-2.5 rounded-full ${((workOrder.assets?.[0]?.status || workOrder.assetStatus) || "online").toLowerCase() === 'offline' ? 'bg-red-500' :
