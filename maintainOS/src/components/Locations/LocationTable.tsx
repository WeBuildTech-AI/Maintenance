...
// --- Constants ---
const STORAGE_KEY_LOCATION_COLUMNS = "location_table_visible_columns";
...
// ✅ FIX 1: Renamed keys to have Spaces
...
const columnConfig: {
  ...
  Vendor: {
    dataIndex: "vendor",
    width: 150,
    sorter: (a, b) => 0, // Disable sort for array
  },
  // ✅ FIX 2: Updated Config Keys
  "Created At": {
    dataIndex: "createdAt",
    width: 150,
    sorter: (a, b) =>
...
export function LocationTable({
  ...
  const [visibleColumns, setVisibleColumns] = useState<string[]>(() => {
    if (typeof window !== "undefined") {
      try {
        const saved = localStorage.getItem(STORAGE_KEY_LOCATION_COLUMNS);
        if (saved) {
        ...
        console.error("Error parsing location column settings:", error);
      }
    }
  ...
  const [selectedLocationTableData, setSelectedLocationTableData] =
    useState<any>([]);
  ...
  const handleApplySettings = (settings: {
    ...
    if (typeof window !== "undefined") {
      ...
      if (hasAllColumns) {
        localStorage.removeItem(STORAGE_KEY_LOCATION_COLUMNS);
      } else {
        localStorage.setItem(
          STORAGE_KEY_LOCATION_COLUMNS,
  ...
  const dataSource = useMemo(() => {
    return location.map((loc) => ({
      ...
      address: loc.address || "—",
      teams: loc.teams || [], 
      // ✅ FIX 5: Map 'vendors' array instead of single 'vendorId'
      vendor: loc.vendors || [], 
