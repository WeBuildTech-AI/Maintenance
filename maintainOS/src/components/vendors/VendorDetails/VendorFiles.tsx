...
export const VendorFiles: React.FC<{ vendor: Vendor }> = ({ vendor }) => {
  const dispatch = useDispatch<AppDispatch>();
  const [loadingKeys, setLoadingKeys] = useState<string[]>([]);
  const [isLoading, setIsLoading] = useState<boolean>(false);

  const files = vendor.vendorDocs || [];
  ...
  if (!files?.length)
    return (
      <p className="text-sm text-gray-500 italic">No vendor files uploaded.</p>
    );
