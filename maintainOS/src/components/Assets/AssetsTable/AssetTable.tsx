...
export function AssetTable({
  ...
  // --- 1. Production Level State Initialization (Updated) ---
  const [visibleColumns, setVisibleColumns] = useState<string[]>(() => {
    if (typeof window !== "undefined") {
      try {
        const saved = localStorage.getItem(STORAGE_KEY_ASSET_COLUMNS);
        if (saved) {
        ...
        console.error("Error parsing asset column settings:", error);
      }
    }
  ...
  // --- 2. OPTIMIZED SETTINGS HANDLER (Updated) ---
  const handleApplySettings = (settings: {
    ...
    if (typeof window !== "undefined") {
      ...
      if (hasAllColumns) {
        // If all columns are selected, remove from storage (default state)
        localStorage.removeItem(STORAGE_KEY_ASSET_COLUMNS);
      } else {
        // Otherwise save the custom configuration
        localStorage.setItem(
          STORAGE_KEY_ASSET_COLUMNS,
