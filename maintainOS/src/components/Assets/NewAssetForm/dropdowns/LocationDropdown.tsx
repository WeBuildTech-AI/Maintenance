...
interface LocationDropdownProps {
  ...
  setSelectedLocation: (val: string) => void; // expose selection
}
    ...
    <div className="relative" ref={LocationRef}>
      ...
      {locationOpen && (
        <div className="absolute left-0 top-full mt-1 w-full rounded-md border bg-white shadow-lg z-50">
          {/* CTA */}
          <ul className="absolute left-0 top-full mt-1 w-full rounded-md border bg-white shadow-lg z-50">
            {/* {locations.map((loc) => (
              <li
                key={loc}
          ...
          <div
            onClick={() => navigate("/locations")} // ✅ Navigate works now
