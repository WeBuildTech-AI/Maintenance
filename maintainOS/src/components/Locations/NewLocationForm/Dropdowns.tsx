    ...
    if (open && !hasFetched) {
      ...
      const fetchAllData = async () => {
        try {
          ...
          let res;
          if (stage === "vendors") {
            res = await vendorService.fetchVendorName(1000, 1, 0, ""); // Example: fetching up to 1000 items
          } else if (stage === "teams") {
            res = await teamService.fetchTeamsName(1000, 1, 0, "");
          } else {
            res = await locationService.fetchLocationsName(1000, 1, 0, "");
          }
          setAllOptions(res.data); // Saara data state mein save karein
