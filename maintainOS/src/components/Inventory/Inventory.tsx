...
export function Inventory() {
  ...
  const stockStatus = useMemo(() => {
    ...
    const delta = selectedPart.unitsInStock - selectedPart.minInStock;
    return { ok: delta >= 0, delta };
  }, [selectedPart]);
  ...
  const createItem = () => {
    ...
    dispatch(createPart(item))
      ...
      .then((createdPart) => {
        ...
        // update UI with new part
        setInventoryData((s) => [createdPart, ...s]);
        setIsCreatingInventory(false);
    ...
    <div className="flex flex-col h-full min-h-0">
      ...
      {viewMode === "table" ? (
        <PartTable
          ...
          setSelectedId={(id) => {
            const found = inventoryData.find((it) => it.id === id) || null;
              ...
              <div className="flex-1 overflow-y-auto min-h-0">
                {inventoryData?.map((it) => (
                  <PartCard
                    ...
                    inventoryData={inventoryData}
                    item={it}
                    selected={selectedPart?.id === it.id}
                    onSelect={() => {
                      setSelectedPart(it); // ✅ full object stored
                      setIsCreatingInventory(false);
                    }}
                  />
                ))}
