      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <label htmlFor="createOnlyIfClosed" className="text-sm text-gray-700">
            Create only if previous Work Order generated from Automation is
            closed
          </label>
        </div>
      </div>
  ...
  const automations: Automation[] = [
    {
      id: 1,
      name: "Automation 1",
      description: 'When asset status = "Critical Failure", create urgent WO',
      status: "enabled",
    },
    {
      id: 2,
      name: "Automation 2",
      description: "When WO SLA < 2 hours, notify manager",
      status: "enabled",
    },
    {
      id: 3,
      name: "Automation 3",
      description: "Escalation workflow for overdue maintenance tasks",
      status: "disabled",
    },
  ];

  // Filter based on tab
  const filtered = automations.filter((a) => a.status === tab);

  // Reset selection when switching tabs
  const handleTabChange = (v: string) => {
    setTab(v as "enabled" | "disabled");
    setSelected(null);
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              {filtered.length > 0 ? (
                filtered.map((auto) => (
                  <div
                    ...
                    className={`flex items-center justify-between p-4 cursor-pointer border-b hover:bg-muted/40 ${
                      selected?.id === auto.id ? "bg-muted/40" : ""
                    }`}
                  >
                    <div>
                    ...
                    <div className="flex items-center gap-2">
                      <Badge variant="default">
                        {auto.status === "enabled" ? "Active" : "Disabled"}
                      </Badge>
        ...
        {selected ? (
          ...
          <>
            <Card>
              <CardHeader>
                <CardTitle>selected  Builder</CardTitle>
                <CardDescription>
                  Create automated workflows with triggers and actions
                </CardDescription>
              </CardHeader>
